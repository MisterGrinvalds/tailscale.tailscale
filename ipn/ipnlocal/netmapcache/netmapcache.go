...
type Cache struct {
	...
	// netmap. This is used to prune keys that are no longer reerenced after an update.
...
func (c *Cache) writeJSON(ctx context.Context, key string, v any) error {
	...
	if ok && bytes.Equal(j, last.contents) {
		return nil
	}
...
func (c *Cache) readJSON(ctx context.Context, key string, value any) error {
	...
	c.wantKeys.Add(key)
	c.lastWrote[key] = lastWrote{contents: data, at: time.Now()}
	return nil
}
